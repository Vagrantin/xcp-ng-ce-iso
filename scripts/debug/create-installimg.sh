...
# ── DEBUG: environment at script start ────────────────────────────────────────
echo "══ DEBUG [create-installimg] ════════════════════════════════════════"
echo "  PWD         : $(pwd)"
echo "  TMPDIR      : ${TMPDIR}"
echo "  ROOTFS      : ${ROOTFS}"
echo "  YUMCONF     : ${YUMCONF}"
echo "  YUMREPOSD   : ${YUMREPOSD}"
echo "  DIST        : ${DIST}"
echo "  SRCURL      : ${SRCURL}"
echo "  CUSTOM_REPOS: ${!CUSTOM_REPOS[@]}"
for k in "${!CUSTOM_REPOS[@]}"; do
    echo "    ${k} -> ${CUSTOM_REPOS[$k]}"
done
echo ""
echo "── YUMCONF contents ─────────────────────────────────────────────────"
cat "${YUMCONF}"
echo ""
echo "── /etc/pki/rpm-gpg/ on container ──────────────────────────────────"
ls -la /etc/pki/rpm-gpg/ 2>/dev/null || echo "  (directory missing or empty)"
echo "── container RPM DB gpg-pubkey entries ──────────────────────────────"
rpm -qa gpg-pubkey 2>/dev/null || echo "  (none)"
echo "══ END DEBUG (pre-setup) ═════════════════════════════════════════════"
# ─────────────────────────────────────────────────────────────────────────────

setup_yum_repos "${YUMFLAGS[@]}"

# ── DEBUG: repo files written by setup_yum_repos ──────────────────────────────
echo "══ DEBUG [post-setup_yum_repos] ══════════════════════════════════════"
echo "── .repo files in YUMREPOSD (${YUMREPOSD}) ─────────────────────────"
for f in "${YUMREPOSD}"/*.repo; do
    echo "  ── ${f} ──"
    cat "${f}"
    echo ""
done
echo "══ END DEBUG (post-setup_yum_repos) ══════════════════════════════════"
# ─────────────────────────────────────────────────────────────────────────────
...
# ── DEBUG: what landed in ROOTFS RPM DB after yum install ─────────────────────
echo "══ DEBUG [post-yum-install] ══════════════════════════════════════════"
echo "── All packages in ROOTFS RPM DB ────────────────────────────────────"
rpm --root="${ROOTFS}" -qa 2>/dev/null | sort || echo "  (rpm -qa failed)"
echo ""
echo "── gpg-pubkey records in ROOTFS (this must be non-empty for erase to work) ──"
rpm --root="${ROOTFS}" -qa gpg-pubkey 2>/dev/null \
    && echo "  ✅ gpg-pubkey found" \
    || echo "  ❌ NO gpg-pubkey in ROOTFS — rpm --erase will fail next"
echo ""
echo "── ROOTFS /var/lib/rpm/ contents ────────────────────────────────────"
ls -la "${ROOTFS}/var/lib/rpm/" 2>/dev/null || echo "  (rpm db dir missing)"
echo ""
echo "── ROOTFS /etc/pki/rpm-gpg/ ─────────────────────────────────────────"
ls -la "${ROOTFS}/etc/pki/rpm-gpg/" 2>/dev/null || echo "  (missing)"
echo "══ END DEBUG (post-yum-install) ══════════════════════════════════════"
# ─────────────────────────────────────────────────────────────────────────────

### removal of abusively-pulled packages (list manually extracted as
### packages not in 8.2.1 install.img)

# ── DEBUG: attempt erase with verbose rpm output ──────────────────────────────
echo "══ DEBUG [pre-erase] ═════════════════════════════════════════════════"
echo "── Attempting rpm --erase of: binutils dracut gpg-pubkey pkgconfig xen-hypervisor"
rpm --root="${ROOTFS}" -qa gpg-pubkey 2>/dev/null \
    || echo "  ❌ gpg-pubkey STILL missing — erase of gpg-pubkey will error"
echo "══ END DEBUG (pre-erase) ═════════════════════════════════════════════"
# ─────────────────────────────────────────────────────────────────────────────
